    ...
    async def async_run(
        ...
        if "zyd_execute" in run_variables and run_variables["zyd_execute"]:
            ...
            def get_rule_variable(self,entity_id):
                variables = {
                    'this':self._hass.states._states[entity_id].as_dict(),
            ...
            async def run_rule(self,entity_id):
                ...
                print("强制执行",entity_id)
                await self._hass.data["automation"]._entities[entity_id].async_trigger(zyd_variables,None,True)


            def set_entity_state(self,entity_id,new_state):
                attributes = self._hass.states._states[entity_id].as_dict()["attributes"]
                self._hass.states.async_set(entity_id,new_state,attributes,True)
            ...
            print(f"run_variables:{run_variables['this']['entity_id']}")
            from homeassistant.zyd.communicate import zyd_communicate
            cmds = zyd_communicate({"location":"after_condition","entity_id":run_variables['this']["entity_id"]},self._hass.states.async_entity_ids(),self._hass.states._states)
            if cmds:
                for cmd in cmds:
                    if cmd["type"] == "default":
                        continue
