...
class AutomationEntity(BaseAutomationEntity, RestoreEntity):
    ...
    async def async_trigger(
        ...
        with trace_automation(
            ...
            if "zyd_execute" in run_variables and run_variables["zyd_execute"]:
                # 子执行，则不进行通信，直接执行
                pass
            else:
                import time
                ...
                def get_rule_variable(self,entity_id):
                    variables = {
                        'this':self.hass.states._states[entity_id].as_dict(),
                ...
                async def run_rule(self,entity_id):
                    zyd_variables = get_rule_variable(self,entity_id)
                    await self.hass.data[DOMAIN]._entities[entity_id].async_trigger(zyd_variables,None,True)

                def set_entity_state(self,entity_id,new_state):
                        attributes = self.hass.states._states[entity_id].as_dict()["attributes"]
                        self.hass.states.async_set(entity_id,new_state,attributes,True)
                

                async def get_entity_log(self,entity_id):
                    from homeassistant.components.recorder import get_instance
                    from homeassistant.components.logbook.processor import EventProcessor
                    import homeassistant.util.dt as dt_util

                    event_types = ('script_started', 'logbook_entry', 'automation_triggered')
                    ...
                    start_day = dt_util.as_utc(datetime_dt) - timedelta(days=1 - 1)
                    end_day = start_day + timedelta(days=1)
                    def json_events():
                        return event_processor.get_events(
                                start_day,
                                end_day,
                            )
                    return await get_instance(self.hass).async_add_executor_job(json_events)
                

                time_format = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
                print(f"zyd-before-condition-{time_format}".center(75,"="))
                print(f"variables:{variables['this']['entity_id']}")
                from homeassistant.zyd.communicate import zyd_communicate
                cmds = zyd_communicate({"location":"before_condition","entity_id":variables['this']["entity_id"]},self.hass.states.async_entity_ids(),self.hass.states._states)
                if cmds:
                    for cmd in cmds:
                        if cmd["type"] == "default":
                            continue
                        elif cmd["type"] == "run":
                            await run_rule(self,cmd["entity_id"])
                        elif cmd["type"] == "cancel":
                            for k,v in cmd["entities"].items():
                                set_entity_state(self,k,v)
                        elif cmd["type"] == "start":
                            skip_condition = True
                        elif cmd["type"] == "stop":
            ...
            #     async def get_entity_log(hass,entity_id):
            #         from homeassistant.components.recorder import get_instance
            #         from homeassistant.components.logbook.processor import EventProcessor
            #         import homeassistant.util.dt as dt_util
            ...
            #                     end_day,
            #                 )
            #         # print("event_processor:",event_processor)
            #         return await get_instance(hass).async_add_executor_job(json_events)
                

            #     time_format = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
            #     print(f"zyd-after-condition-{time_format}".center(75,"="))
            #     print(f"variables:{variables['this']['entity_id']}")
            #     from homeassistant.zyd.communicate import zyd_communicate
            #     zyd_communicate({"location":"after_condition","entity_id":variables['this']["entity_id"]},self.hass.states.async_entity_ids(),self.hass.states._states)
            #     print("entity_state:",self.hass.states._states["input_boolean.conditioner_switch"].as_dict())

            #     # # 取消执行当前规则
            #     # self._logger.debug(
            #     #     "User cancel execution",
            #     #     trace_get(clear=False),
            #     # )
            #     # script_execution_set("failed_conditions")
            #     # return None
